...
R3-06 新增独立运行控制面，分别表达暂停 AI、停止新开仓和紧急模式。控制事实保存在 SQLite FULL/WAL
不可变事件链中；Telegram 命令必须同时通过 user/chat allowlist。AI 暂停只阻止 provider 网络请求，不停止
只读调度和风险观察；停止新开仓会叠加到调度快照及 OPEN/ADD 执行前复验，控制状态不可读时 fail-closed，
但安全退出始终保留。紧急模式额外要求取消待处理入场并人工复核，R3-06 不直接发送撤单或平仓指令；订单
处置和恢复证据由 R4 ExecutionGateway 负责。详细合同见
[ADR-0017](decisions/0017-operational-control-surface.md)。
...
- alphaMind Operational Control Store：暂停 AI、停止新开仓和紧急模式的不可变控制事件与当前投影，不保存订单/成交权威；
