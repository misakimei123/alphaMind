...
## R2-06 演进说明（2026-07-21）

- ADR-0008 已将活动 `DecisionContext` 显式升级为 v2，并把 feature version 升为 `r2-06-v2`；v1
  不会被 binder 静默补字段。R2-05 在本 ADR 中冻结的 Donchian、ATR、EMA、volume ratio 参数和公式
  保持不变，新版本只在同源已完成 candle 上增加 RSI、ADX、EMA alignment 与位置过滤形态；
- v2 输入 hash 额外绑定 cycle id 和全部扩展参数；Context 仍只携带模型需要的受控 feature 字段，内部
  hash、reason code 和完整参数证据继续留给后续 R2-07 持久化；
- 每个 instrument 仍只有一组 canonical features，多市场/多 timeframe 同时表达的限制没有在 R2-06
  中解除。
