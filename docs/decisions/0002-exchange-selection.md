...
| 元数据 | 内容 |
|---|---|
| 状态 | DONE / 2026-07-18 扩展为 Spot + USDT Linear Perpetual |
| 任务 | P0-02 |
| 决策日期 | 2026-07-15 |
| 目标交易所 | Bybit 国际版 |
| 市场 | Spot |
...
Bybit 国际版和 Freqtrade 唯一交易写入者继续有效。当前 MVP 扩展为：

- Instrument Registry 初始配置 BTC、ETH、SOL、HYPE；
- 同一 instrument 可分别配置 `spot_pair` 与 `futures_pair`；
- Bybit spot + USDT linear perpetual；
- Spot/Futures 使用独立 Freqtrade 配置、Runtime DB、bot identity 和 secret；
- futures 使用 Freqtrade/CCXT `base/quote:settle` 命名、isolated margin 和 one-way mode；
- 全局、标的和交易所三层最大杠杆取最小值；
- futures 优先使用 stoploss on exchange，spot 继续使用 bot-managed stoploss；
- 启动时动态查询 `instruments-info`，确认市场状态、tick、step、最小金额、funding interval 和最大杠杆；配置示例不保证某个市场届时仍存在。

本文原 capability matrix 只证明 BTC/ETH spot v1 底座。R1-03 负责生成 Spot/Linear 机器可读能力快照，R5-05 负责 Bybit Demo/Testnet 合约验证。旧“下一步 P0-03”不再是当前进度；唯一下一任务见 [完整开发计划](../development-plan.md)。
...
P0-02 的 BTC/ETH spot capability 调查状态为 `DONE`，作为历史 v1 证据保留。当前 Bybit 目标已扩展为配置化 Spot/Futures；下一任务以 [完整开发计划](../development-plan.md) 为准。
