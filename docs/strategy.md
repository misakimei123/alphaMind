...
> 2026-07-18 定位修订：本文记录传统量化策略研究和 Donchian 历史基线，不再定义产品主线或任务状态。当前交易权威是“AI 结构化 Action -> Telegram 人工授权 -> 确定性风控 -> Freqtrade 执行”；Donchian、ATR、EMA 等作为 DecisionContext 特征和对照基线。现货、合约、配置化标的和 R0-R6 以 [完整开发计划](development-plan.md) 为准。
...
其中，趋势跟踪和受市场状态约束的均值回归仍是有价值的确定性特征与研究基线，但不再拥有 MVP 唯一决策权。Funding/Basis Carry、BTC-ETH 统计套利、横截面动量和订单簿策略仍属于后续方向；普通 USDT 永续 long/short 执行能力已纳入 R5。
