...
| 制定日期 | 2026-07-15 |
| 最近进度更新 | 2026-07-22 / R3-06 完成暂停 AI、停止新开仓与紧急处置控制面；R3 阶段关闭，下一任务为 R4-01 ExecutionGateway POC |
| 适用范围 | 个人 AI 交易系统；默认每 30 分钟观察；Telegram 人工授权；Bybit 现货与 USDT 永续；配置化 BTC/ETH/SOL/HYPE；Freqtrade 执行 |
> 当前阶段：已完成的 P0-01 至 P2-06、P3-01 至 P3-04 作为可复用研究、风险、审计和数据库底座保留。项目于 2026-07-18 重新定基线：AI 决策、新闻输入、Telegram 审批和批准后自动执行成为 MVP 主链；币种、市场、周期和杠杆改为配置化；现货与 Bybit USDT 永续均纳入 MVP。R0-01 至 R0-04 已完成配置、Schema、模型、Prompt 和首批新闻源合同；R1-01 至 R1-06 已完成配置化账户观察底座；R2-01 至 R2-07、R3-01 至 R3-06 已完成并关闭；当前下一任务是 R4-01 Freqtrade ExecutionGateway POC。R0-05 的真实资金参数不阻塞 R1-R3 dry-run。`development-plan.md` 是唯一规范与进度收口，其他文档不能维护独立任务状态
...
| 策略特征 | Donchian、ATR、EMA、成交量为核心；R2-06 增加 RSI、ADX、EMA 排列和位置过滤后的 K 线形态 | 全部作为 AI 只读观察特征和传统基线，不直接生成可执行数量或绕过审批、确定性风控；Bollinger Bands 暂不进入必做范围 |
...
- Donchian、ATR、EMA、成交量、RSI、ADX、EMA 排列及关键位置过滤后的 K 线形态等确定性 AI 观察特征；
...
- `R2-06`（`DONE`，新增）：落实 ADR-0008，在 R2-05 的同源 point-in-time 特征快照上增加 RSI、
  ...
  决策质量对照；
- `R2-07`（`DONE`，原 R2-06）：保存 HOLD、候选动作、模型错误、model/prompt/config/schema
  版本和输入 hash。
...
- R2-05 已能从同一批已完成 K 线确定性生成带来源时间、timeframe 和 freshness 证据的核心特征；
- 在修改 Schema 或计算代码前，先在 ADR-0008 补齐 RSI/ADX 周期与平滑口径、EMA 排列阈值、关键位置
  容差、形态冲突优先级，以及 ATR/实体/振幅为零、warmup、缺 candle 和指标不可用时的行为；
- 明确 DecisionContext 的版本迁移策略；禁止在不变更 schema/version 证据的情况下静默改变冻结合同语义。

实现范围与优先级：

1. **P0 必做**：RSI 原始值、EMA alignment、Schema/fixture/Prompt 同步、point-in-time 与
   lookahead 边界；RSI 必须限制在 `[0, 100]`，不可用时使用合同明确允许的 `null`，不得伪造中性值；
2. **P1 必做**：ADX 原始值；ADX 同样限制在 `[0, 100]`，计算周期、平滑方法和 warmup
   必须固定并可由独立实现复算；
3. **形态必做**：ADR-0008 列出的单根/双根形态由 Python 硬规则识别；只有接近 Donchian 上下轨或
   `ema_slow` 的形态才可上报。位置阈值必须使用预先冻结、可复算的 ATR 或比例容差，不能用模型主观判断；
4. **语义合同**：`ema_alignment`、`candlestick_pattern` 和 `pattern_semantic` 使用受控枚举或稳定代码映射；
   nullable enum 必须真实接受 `null`，不接受自由文本替代内部标签；多个形态同时命中时按冻结优先级只输出
   一个稳定结果，未命中或被位置过滤时输出 `null`；
5. **明确延期**：Bollinger Bands 仅是 ADR-0008 的 P1 可选项，本任务不实现；若以后引入，必须单独说明
   相对 Donchian/ATR 的增量信息、共线性风险和消融证据，不得顺手扩展。

产物：

- 指标/形态纯计算模块与 Feature Builder 集成，所有输出绑定 candle close time、timeframe、计算参数和
  当前 cycle，重复输入产生相同规范化 Context/hash；
- 版本化 DecisionContext Schema 与合法/非法 fixture；修正数值范围、nullable enum、受控语义映射和
  `additionalProperties: false` 边界，并同步 binder 的支持版本与跨对象测试；
- 版本化 Prompt 及 AI profile 中的新 SHA-256，要求模型交叉检查突破、动量、趋势强度、成交量和位置形态；
  指标冲突时优先 `HOLD` 并显式记录冲突，但不得要求模型披露隐藏思维链，也不得把形态标签写成必然买卖指令；
- 至少覆盖趋势、震荡、暴跌、阴跌、关键位置反转、非关键位置噪音、warmup/缺口/零振幅/极小实体和
  多形态冲突的冻结离线 Context 集合，以及扩展前后的版本化 AI dry-run 对照报告。

验证与完成标准：

- unit/property/boundary tests 独立复算 RSI、ADX、EMA alignment、影线/实体与双 candle 包含关系；所有
  指标只读取当前及更早已完成 candle，未完成 candle、未来数据或错位 timeframe 必须 fail-closed；
- contract tests 证明新旧版本迁移策略、字段范围、`null`、未知 enum、Prompt hash 和 fixture 均符合合同；
- 对目标指标运行锁定环境下的 recursive/lookahead 或等价前缀一致性检查，证明增加未来 candle 不改变历史输出；
- AI dry-run 对照至少报告 Schema 合法率、应 HOLD 场景命中率、关键冲突引用率、非法/越权动作率和
  token/成本变化。离线 rationale 或少量样例不能证明“胜率提升”，不得以主观观感代替可复核指标；
- 聚焦测试、相关全仓门禁和 `git diff --check` 通过后状态进入 `READY_TO_VERIFY`；只有 Schema、Prompt、
  fixture、计算链和对照证据全部闭环后才能标记 `DONE`。
...
R2-05 完成证据（2026-07-21）：

- `src/alphamind/candles.py` 新增交易所无关的 `CompletedCandle` 与固定 timeframe 合同；
  `src/alphamind/market/bybit.py` 新增无认证 `BybitKlineClient`，固定官方 HTTPS 端点 allowlist、
  timeout、Content-Type/响应大小、category/symbol/行结构与 OHLCV 校验，并按官方语义剔除仍未闭合的
  最后一根 candle。响应失败、身份错配、重复时间或非法价格均 fail-closed，不读取 API key；
- `src/alphamind/decision/features.py` 实现版本化 `r2-05-v1` 纯计算：Donchian 20/10 明确排除当前
  candle，ATR(20) 使用 Wilder/RMA，EMA 使用 20/50 简单均值播种后递推，volume ratio 使用当前成交量
  除以前 20 根均量；全部使用 Decimal 并以 half-even 收口至 8 位小数，同输入、参数和版本形成稳定
  SHA-256。warmup、零成交量、缺口、future/stale candle、timeframe 错位和输入上限均输出稳定 reason
  code 与 `null`，不得伪造中性或部分可信特征；
- `CoreFeatureSnapshot.to_context_features()` 只输出 DecisionContext v1 已冻结的七个 features 字段，
  并由真实 `DecisionContractBinder` 测试证明可绑定；内部 feature version、输入 hash 和 reason code 不以
  无版本字段泄漏到 Context。新增 ADR-0010 冻结 OHLCV 来源、参数、市场选择和 point-in-time 边界；
  R2-06 的 RSI/ADX/EMA alignment/K 线形态未提前实现；
- 新增 6 项测试，R2-05 聚焦测试连同 market/context 既有回归共 37 项通过；覆盖确定性数值、Context
  绑定、Donchian 当前 candle 排除、ATR/EMA 当前 close 纳入、warmup、零量、缺口、future、stale、
  timeframe mismatch、未闭合 candle 过滤、倒序归一化、市场身份、重复时间和非法 OHLCV；
- 全仓 `pytest` 372 项通过，strict mypy 检查 68 个 source/script 文件，Ruff check/format 覆盖 104 个
  Python 文件，repository scan 检查 349 个文件；双市场 pairlist、capability 完整性、有效配置
  `execution_ready=true`、`uv lock --check` 和 `git diff --check` 均通过；
- 2026-07-21T02:51:48Z 使用 Bybit 主网公共 Kline 端点完成一次真实无认证只读冒烟：BTC/ETH/SOL/HYPE
  各取得 199 根已完成 30m candle，最后闭合时间均为 02:30:00Z，四个 `r2-05-v1` 快照均
  `ready=true`、reason code 为空；对应输入 hash 分别为
  `03c080157ee3c321010fc499a1215d371909a36b633fc6f44859e5acfd50112c`、
  `3eb478d370bfea60a67f52e365800088765208137049657a9a1e65820b2bec40`、
...
R2-07 完成证据（2026-07-21）：

- [ADR-0011](decisions/0011-ai-decision-journal.md) 冻结 Decision Journal 与 UsageLedger、Audit
  Outbox/Audit DB、Freqtrade Runtime DB 的所有权边界：Journal 是已验证 AI 周期终态事实源，不声明
  Telegram 批准、风险定仓、订单或成交；Audit Writer 是否在线不影响 R3 后续读取候选动作；
- `src/alphamind/ai/journal.py` 新增独立 SQLite FULL/WAL append-only Journal。每个 `cycle_id` 只允许一个
  `HOLD`、`CANDIDATE_ACTIONS` 或 `MODEL_ERROR` 终态；相同内容幂等，异文拒绝覆盖；记录正文使用
  canonical JSON 与整体 SHA-256，读取时复核正文、索引列、outcome/动作一致性和版本/hash 绑定，篡改
  fail-closed；
- 每条终态绑定 environment、profile/model ID、prompt ID/version/SHA-256、effective config SHA-256、
  DecisionContext 输入 SHA-256、NewsItem/DecisionContext/ModelDecision/TradeAction Schema 版本以及候选
  决策正文 SHA-256。模型错误只保存安全 error code、provider ID、usage 和逐动作拒绝摘要，不保存异常消息、API
  key、原始 Prompt 或未经 binder 的原始响应；
- `OpenAICompatibleProvider` 在返回 HOLD/候选动作/模型错误前统一追加 Journal；Journal 不可写或 cycle
  内容冲突时丢弃内存候选动作并返回 `decision_persistence_error` / `HOLD_ONLY`。`run-ai-decision` 新增
  仓库内 `--decision-db` 路径，默认 `user_data/state/ai-decisions.sqlite`，路径越界继续拒绝；
- 新增 5 项 Journal 单元测试并扩展 provider 集成测试，聚焦共 29 项通过，覆盖 HOLD、合法候选、全部
  拒绝/模型错误、版本与 hash、幂等、冲突、防篡改、写入失败降级和 secret/error detail 不外泄；
- 全仓 `pytest` 411 项通过，strict mypy 检查 70 个 source/script 文件，Ruff check/format 覆盖 109 个
  Python 文件，repository scan 检查 359 个文件，`uv lock --check` 和 `git diff --check` 通过；本任务
  未调用真实 LLM、未读取 API key、未发送 Telegram，也未创建、修改或取消订单。R2-07 与 R2 阶段据此
  转为 `DONE`，下一任务为 R3-01 Proposal Store。
...
- `R3-06`（`DONE`）：暂停 AI、停止新开仓和紧急处置入口。

完成标准：一次批准只产生一个 `QUEUED` execution fact；拒绝、过期、非白名单和控制面阻断不会进入执行队列。fake executor 的一次性执行门禁由 R4-01 验证。
...
R3-02 完成证据（2026-07-21）：

- [ADR-0013](decisions/0013-python-telegram-bot-approval-adapter.md) 决定使用主流
  `python-telegram-bot` 22.x；当前锁定 22.8，并按其依赖合同把 `httpx` 收紧到 `<0.29`。适配器使用
  async `Bot` 生命周期和 `send_message`、`edit_message_text`、`answer_callback_query` 正式 API；
- `src/alphamind/approval/telegram.py` 新增纯文本概览/详情/终态渲染、原生 `InlineKeyboardMarkup`、
  Telegram 客户端和 Proposal Store 适配器。展示覆盖标的、市场、方向、动作、入场、止损、止盈、杠杆、
  有效期、理由、新闻引用及风险，并明确批准只形成授权、尚未执行；
- 按钮 callback data 只含操作和 proposal ID，构造时强制 64 UTF-8 bytes 上限，不包含原始 nonce、
  user/chat ID 或完整 Action。发送成功后才迁移到 `PENDING_APPROVAL`；Store 失败时尽力撤下按钮，发送失败
  保持 `VALIDATED`；详情不迁移状态，批准/拒绝/过期会更新 Store、编辑原消息并移除按钮；
- 回调处理只接受 R3-03 未来产生的 `VerifiedTelegramCallback`，并先 ACK callback。R3-02 不解析或信任
  原始 Update，不读取真实白名单、ID 或 nonce；白名单、nonce 一次性消费、原始 callback 解析和重复点击
  安全闭环仍是 R3-03；
- 新增 6 项 Telegram 聚焦测试，覆盖原生按钮及 payload 上限、async Bot 调用、错误脱敏、发送失败、详情、
  批准、拒绝和过期；与 Proposal Store 合计 15 项聚焦测试通过；
- 全仓 `pytest` 427 项通过，strict mypy 检查 73 个 source/script 文件，Ruff check/format 覆盖 114 个
...
R3-04 完成证据（2026-07-21）：

- [ADR-0015](decisions/0015-execution-preflight-revalidation.md) 冻结 `APPROVED -> REVALIDATING ->
  QUEUED/EXPIRED/CANCELLED` 与 R4 ExecutionGateway 的边界：批准只是人工授权，只有绑定最新可信快照并
  通过全部重新校验的 Action 才能生成执行队列事实，`QUEUED` 仍不表示订单已提交；
- `src/alphamind/approval/revalidation.py` 新增 `ActionRevalidator` 与 `RevalidationCoordinator`。校验只
  接受已绑定 DecisionContext、已验证 RiskSnapshot 和有效配置中的 Instrument Registry/Market Capability；
  Context 与 RiskSnapshot 的 id、NAV、spot 可用余额、futures 可用保证金、risk state 和 open order ID
  集合必须一致，输入缺失、损坏或混用均 fail-closed；
- `OPEN/ADD` 重新检查 entry 仍允许、价格仍在用户批准区间、仓位未发生冲突、余额/保证金满足当前最小
  notional、没有同方向风险增加挂单；安全退出动作重新检查仓位、方向、目标订单/保护单和 safe-exit 权限。
  所有动作重新检查市场可用性、tick/step/minimum quantity/minimum notional 及 futures leverage 上限；
- Proposal Store 就地迁移旧 R3-01 SQLite CHECK，保留原审批事件和外键；重新校验通过时只写一个确定性
  execution ID，并绑定 `context_sha256`、`risk_snapshot_id`、market capability source SHA-256、校验时间和
  空 order ID 集合。失败/过期不创建 execution 详情，重复处理已完成 Proposal 只返回既有终态；
- 新增 7 项 R3-04 测试，与 R3-01 至 R3-03 合计 38 项审批链聚焦测试通过，覆盖一次排队与重复消费、价格
  漂移、仓位/余额/挂单并发变化、RiskSnapshot/Context 不一致、entry 风险阻断、杠杆/tick 规则变化、TTL、
  风险快照缺失和旧 SQLite 原库迁移；
- 全仓 `pytest` 450 项通过，strict mypy 检查 75 个 source/script 文件，Ruff check/format 覆盖 117 个
...
R3-06 完成证据（2026-07-22）：

- [ADR-0017](decisions/0017-operational-control-surface.md) 冻结三类控制语义：暂停 AI 只停止模型网络请求，
  停止新开仓阻断 OPEN/ADD，紧急模式同时执行前两项并标记取消待处理入场与人工复核；任何状态都保留
  安全退出，R3-06 不声称已经撤单、减仓或平仓；
- 新增 `OperationalControlSnapshot v1`、仓库内独立 `operations.control_store_path` 和 SQLite FULL/WAL
  控制库。状态从不可变事件顺序重放，事件与投影 hash 每次读取均复核；同文重放幂等、异文冲突、历史
  篡改和不可读状态均 fail-closed；
- Telegram 控制入口只接受普通文本消息中的精确命令，并复用当前环境 user/chat 双重 allowlist；原始身份
  不落库。`/resume_entries` 需要最新可信 RiskSnapshot 仍允许入场；紧急模式不能由普通 resume 命令解除；
- AI CLI 在 provider 创建和网络请求前读取控制状态；调度器继续只读观察，并把停止新开仓/紧急模式叠加
  到有效 RiskSnapshot 视图。执行前复验对 OPEN/ADD 读取控制状态，停止或不可读均取消候选，不影响
  REDUCE/CLOSE 等安全退出；
- 新增 12 项运行控制/配置/Schema/调度/复验回归；全仓 `pytest` 471 项通过，strict mypy 检查 79 个
  source/script 文件，Ruff check/format 覆盖 123 个 Python 文件，repository scan 检查 383 个文件，
  R2-06 冻结报告一致性、`uv lock --check` 和 `git diff --check` 通过；
- 测试使用本地 SQLite、fake Telegram client 和离线 RiskSnapshot，未连接真实 Telegram、Bybit、
  Freqtrade 或 LLM provider，未读取真实凭据，也未创建、修改或取消订单。R3-06 与 R3 阶段据此转为
  `DONE`，下一任务为 R4-01 Freqtrade ExecutionGateway POC。
...
| AI feature/context | 独立复算 + boundary/property + contract fixture + prefix/lookahead + AI dry-run 对照 | 只用已完成 K 线、同输入同 hash、范围/空值/语义受控、非关键位置形态被过滤、行为与成本变化可复核 |
...
| 38 | R3-01 至 R3-06 Telegram 授权 | DONE | Proposal、Telegram 授权、重新校验、通知和运行控制面均已完成 |
...
2026-07-22 当前顺序：旧 P2-07/P2-08 和 P3-05 之后的原顺序不再决定下一任务；R0-05 的真实资金参数保持 `BLOCKED` 但不阻止 dry-run 开发；R1-01 至 R1-06、R2-01 至 R2-07、R3-01 至 R3-06 已完成。当前唯一下一任务是 R4-01（Freqtrade ExecutionGateway POC）。Proposal Store 保存逐 Action 授权、重新校验与执行队列绑定，notification outbox 只保存待投递的受限事实，Operational Control Store 只保存运行控制事实，三者均不拥有订单或成交权威；真实账户、API 写权限和资金动作仍必须等待 R4-R6 的对应条件。
...
- `python-telegram-bot` 22.8 提供 fully async `Bot` API；文本消息上限为 4096 characters，inline button
  `callback_data` 上限为 64 UTF-8 bytes，callback query 必须通过 `answer_callback_query` 响应；
