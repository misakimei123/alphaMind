...
> 2026-07-18 适用范围修订：本文继续约束 Donchian 等传统策略的严格研究声明，但不维护产品任务状态，也不以 Final Holdout、独立评审或固定 90 天 Paper 阻止 AI、Telegram、dry-run 和 Demo/Testnet 开发。当前产品范围和 R0-R6 以 [完整开发计划](development-plan.md) 为准。
...
第一阶段固定研究 ADR-0004 选择的 4h Donchian Breakout；不得再实现 Dual Moving Average 后择优，也不得在查看目标数据结果后改换首个策略。
...
ADR-0005 已将首个候选的数据区间固定为 Bybit spot `BTC/USDT`、`ETH/USDT` 的
`[2022-01-01, 2026-07-01)` UTC OHLCV；开发池在 `2025-07-01` 截止，之后连续
12 个月为一次性 Final Holdout。4h 为主周期，1d 只做基线稳健性复测。实际文件
hash 在 P1-03 下载后登记，但 SHA-256 算法、不可变 snapshot 和禁止原地覆盖的规则
已经冻结。
...
首个 Donchian 候选已经在 ADR-0005 固定为 expanding train、6 个月 validation、
6 个月步长，共三个 fold：2024H1、2024H2、2025H1 为依次独立的 validation 区间。
...
首个候选原 Final Holdout `[2025-07-01, 2026-07-01)` UTC 已因 P1-03 完整分区
OHLC/volume 扫描而严格降级为 `DEGRADED_TO_DEVELOPMENT`，`access_count=1`。该区间不得用于
P2-07；新 Final Holdout 必须等待新的未见连续数据形成，并在访问前重新预注册和审批。
...
- 本条只适用于 Donchian v1 历史研究基线；AI MVP 的合约杠杆按开发计划 R5/R6 单独验证；
- 单笔风险不超过 NAV 的 0.25%；
- 单日亏损达到 1% 自动暂停；
- 单周亏损达到 3% 进入人工复核；
- 从历史峰值回撤达到 5% 自动停止策略；
