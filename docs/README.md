...
本文档目录记录项目当前的目标架构和实施规范。仓库目前尚未包含具体实现，因此文档描述的是后续开发必须遵守的设计边界，而不是对现有代码的说明。
...
| 项目 | 当前基线 |
|---|---|
| 市场 | 单一交易所现货；具体交易所必须在 Phase 0 结合司法辖区、API 能力和账户条件确定 |
| 标的 | BTC/USDT、ETH/USDT |
| 方向 | long/flat，不使用杠杆 |
| 第一条策略 | 4h Donchian Breakout 或 Dual Moving Average；二选一后固定规则 |
| 第二条策略 | Regime-filtered Mean Reversion，必须独立完成验证 |
| 研究和 dry-run | Freqtrade |
| 交易所接入 | Freqtrade 内部 CCXT 能力优先；确有差异时才增加原生适配 |
| AI | 不进入实时资金决策链，不是 MVP 晋升的前置依赖 |

具体交易所、初始资金、风险预算、责任人和部署区域仍是 Phase 0 的待决事项。在这些决策形成版本化记录前，不进入实现阶段。
